 ...
 * Add a bookmark
 */
export async function addBookmark(memberstackId, resourceType, resourceId) {
  try {
 ...
 * Remove a bookmark
 */
export async function removeBookmark(memberstackId, resourceType, resourceId) {
  try {
 ...
 */
export async function getUserBookmarks(memberstackId, { resourceType = null, page = 1, limit = 20 } = {}) {
  try {
    const skip = (page - 1) * limit;
 ...
 */
export async function isBookmarked(memberstackId, resourceType, resourceId) {
  try {
    const bookmark = await prisma.userBookmark.findUnique({
      where: {
        memberstackId_resourceType_resourceId: {
          memberstackId,
          resourceType,
          resourceId
        }
 ...
 */
export async function getUserBookmarkIds(memberstackId) {
  try {
 ...
 */
export async function updateReadingProgress(memberstackId, articleId, progress, completed = false) {
  try {
    const result = await prisma.readingProgress.upsert({
      where: {
        memberstackId_articleId: {
          memberstackId,
          articleId
        }
      },
 ...
 */
export async function getUserReadingProgress(memberstackId, articleIds = []) {
  try {
    const where = {
      memberstackId,
